.PHONY: build test clean install run-example clean-example vendor docker-build docker-test backup update-std-package-list version tag-major tag-minor tag-patch list-tags
...
# Build flags
LDFLAGS=-ldflags "\
    -X 'github.com/siyuan-infoblox/go-imports-group/pkg/version.Version=$(VERSION)' \
    -X 'github.com/siyuan-infoblox/go-imports-group/pkg/version.GitCommit=$(GIT_COMMIT)' \
    -X 'github.com/siyuan-infoblox/go-imports-group/pkg/version.GitTag=$(GIT_TAG)' \
    -X 'github.com/siyuan-infoblox/go-imports-group/pkg/version.BuildDate=$(BUILD_DATE)'"
